...
app.delete('/api/delete-document/:id', async (req, res) => {
  try {
    ...
    const correspondingNotDoneDocument = await notDoneCollection.findOne({ _id: new ObjectId(documentId) });
    ...
    if (correspondingNotDoneDocument) {
      ...
      console.log(`Document with ID ${documentId} deleted successfully from evpnotdones`);
    }

    // Find the document to delete in the 'evpattendances' collection
    const documentToDelete = await attendanceCollection.findOne({ _id: new ObjectId(documentId) });
    ...
    // Log success message
    console.log(`Document with ID ${documentId} deleted successfully from evpattendances`);
